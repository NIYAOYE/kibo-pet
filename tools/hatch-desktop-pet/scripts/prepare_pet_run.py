#!/usr/bin/env python3
"""Create a desktop pet run folder, prompts, and imagegen job manifest."""
...
from pet_layout import ATLAS, ROW_DEFS

# (state, row, frames, purpose) tuples, in row order, from the shared layout.
ROWS = [
    (str(e["state"]), int(e["row"]), int(e["frames"]), str(e["purpose"]))
    for e in ROW_DEFS
]
...
STATE_REQUIREMENTS = {
    "idle": [
        ...
        "Do not show waving, walking, jumping, talking, emotional reactions, large gestures, item interactions, or new props.",
        "Feet, base, body, or object anchor should remain planted or nearly planted.",
        "The first and last frames should be very close visually so the loop feels calm and does not pop.",
    ],
    "walk-right": [
        "Show rightward walking through leg/body stride and arm swing only; the pet faces and moves to the right.",
        "Do not draw speed lines, dust clouds, floor shadows, motion trails, or detached motion effects.",
    ],
    "walk-left": [
        "Show leftward walking through leg/body stride and arm swing only; the pet faces and moves to the left.",
        "This is a separately drawn row, not a mirror: keep the halo, hair tint, and any one-sided markings correct for a left-facing pose.",
        "Do not draw speed lines, dust clouds, floor shadows, motion trails, or detached motion effects.",
    ],
    "drag": [
        "Show the pet as if picked up and dangling from above: body lifted, limbs hanging loose, a small startled or wobbling expression.",
        "Do not draw the cursor, a hand, a string, grab lines, motion marks, or any detached effect; only the pet appears.",
        "Keep the pet centered in each slot; convey hanging through body/limb pose, not by drifting toward an edge.",
    ],
    "sleep": [
        "Show sleeping through closed eyes, a relaxed lowered head, and calm settled posture.",
        "Do not draw floating Z letters, speech/thought bubbles, symbols, or any detached effect.",
        "Keep motion minimal: gentle breathing rise/fall only, looping calmly.",
    ],
    "greet": [
        "Show the greeting through arm/paw pose only: arm down, arm raised, a small wave tilt, and return.",
        "Do not draw wave marks, motion arcs, lines, sparkles, symbols, speech bubbles, or floating effects.",
    ],
    "thinking": [
        "Show the pet actively thinking/processing: focused posture, hand near chin or tilted head, small purposeful bobbing.",
        "This is not foot-running: no jogging, sprinting, treadmill motion, raised knees, long steps, pumping arms, or directional travel.",
        "Do not add magnifying glasses, papers, code, UI, gears, question marks, or other new props/symbols unless they already exist in the base pet identity.",
    ],
    "talk": [
        "Show talking through mouth open/close and a slight head bob only.",
        "Do not draw speech bubbles, thought bubbles, text, sound waves, punctuation, or any detached effect.",
    ],
    "happy": [
        "Show happiness through a small celebratory hop, bright smiling expression, and raised arms.",
        "Show vertical motion through body position only; do not draw confetti, detached sparkles, floor shadows, dust, or landing marks.",
    ],
    "sad": [
        "Show sadness through a slumped pose, drooping limbs, downcast or teary-but-dry eyes, and lower body position.",
        "Do not draw red X marks, floating symbols, detached stars, rain clouds, or other loose effects.",
    ],
    "cry": [
        "Show crying through a scrunched/sad face and slumped posture.",
        "Tears are allowed only if attached to or touching the face and kept inside the same frame slot.",
        "Do not draw separate falling tear droplets, floating symbols, detached water splashes, or other loose effects.",
    ],
    "surprised": [
        "Show surprise through wide eyes, a small backward recoil, and a startled body pose.",
        "Do not draw exclamation marks, floating symbols, shock lines, sparkles, or any detached effect.",
    ],
    "love": [
        "Show affection through a blushing happy expression, clasped hands, or a small swoon.",
        "A heart is allowed only if it is opaque, pixel-style, and physically touching or overlapping the pet; otherwise convey love through expression only.",
        "Do not draw detached floating hearts, sparkles, symbols, or other loose effects.",
    ],
}

DIGITAL_PET_STYLE = (
    "Desktop pet sprite style: pixel-art-adjacent low-resolution mascot sprite, "
...
def infer_description(args: argparse.Namespace, reference_paths: list[Path]) -> str:
    ...
    if args.pet_notes.strip():
        return sentence(f"A compact desktop pet: {args.pet_notes}")
    if reference_paths:
        return "A compact desktop pet based on the provided reference image."
    return "A compact original desktop pet ready for animation."
...
def infer_pet_notes(args: argparse.Namespace, reference_paths: list[Path]) -> str:
    ...
    return "a compact original desktop pet"
...
def base_pet_prompt(args: argparse.Namespace) -> str:
    ...
    return f"""Create a single clean reference sprite for a desktop pet named {args.display_name}.
...
def row_prompt(
    ...
    return f"""Create a single horizontal sprite strip for the desktop pet `{args.pet_id}` in the state `{state}`.

Use the attached reference image(s) for pet identity and the attached base pet image as the canonical design. Use the attached layout guide image only for frame count, slot spacing, centering, and safe padding. Simplify any high-resolution reference details into the desktop pet sprite style. Do not simply copy the still reference pose. Generate distinct animation poses that create a readable cycle.
...
def make_jobs(
    ...
    for state, _row, frames, _purpose in ROWS:
        ...
        # walk-left is generated as its own grounded row (no mirroring) so the
        # halo/hair tint stay correct; it uses walk-right as a gait reference so
        # the two directions stay consistent.
        if state == "walk-left":
            depends_on.append("walk-right")
            extra_inputs.append(
                {
                    "path": "decoded/walk-right.png",
                    "role": "rightward gait reference; match cadence and stride, mirror the facing direction",
                }
            )
        jobs.append(
            {
