...
- Art reads as a desktop pet sprite, not a polished illustration or glossy app icon.
...
- The `idle` row should be calm and low-distraction; reject it if it reads as greeting, walking, jumping, talking, working, reacting dramatically, changing props, or making large pose/silhouette changes.
- No important detail is too small to read.
- No frame is clipped by the cell.
- The `sleep`, `thinking`, and emotion rows (`happy`/`sad`/`cry`/`surprised`/`love`) must be visually distinct from ordinary idle.
- `walk-left` must be a genuine left-facing row, not a flipped copy of `walk-right`; check the halo/hair tint and one-sided markings read correctly.
