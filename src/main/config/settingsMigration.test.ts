...
describe('settings v1 → v2 迁移', () => {
  ...
  it('读 v1 文件(无 search)补默认 duckduckgo 并升到 v12', () => {
...
describe('v2 -> v3 迁移(memory)', () => {
  ...
  it('v2 设置(无 memory)加载后补 memory.embedding = null,schemaVersion 升为 12', () => {
    const file = tempFile(JSON.stringify({
      schemaVersion: 2,
      provider: { kind: 'openai-compat', baseURL: 'https://api.deepseek.com/v1', model: 'deepseek-chat' },
      search: { backend: 'tavily' }
...
describe('MVP-12 firecrawl 迁移', () => {
  it('缺失 firecrawl 时补默认 { enabled:false } 且 schemaVersion 升到 12', () => {
    const out = normalizeSettings({
      schemaVersion: 5,
      activePetId: 'luluka',
      provider: { kind: 'anthropic', model: 'claude-haiku-4-5' },
...
describe('desktopControl 迁移', () => {
  it('缺失 desktopControl 时补默认 { enabled:false } 且 schemaVersion 升到 12', () => {
    const out = normalizeSettings({
      schemaVersion: 6,
      activePetId: 'luluka',
      provider: { kind: 'anthropic', model: 'claude-haiku-4-5' },
...
describe('tts 迁移', () => {
  it('缺失 tts 时补齐 DEFAULT_TTS_SETTINGS 且 schemaVersion 升到 12', () => {
    const out = normalizeSettings({
      schemaVersion: 8,
      activePetId: 'luluka',
      provider: { kind: 'anthropic', model: 'claude-haiku-4-5' },
  ...
  it('v9 设置(tts 无 textSplit)→ 补默认 smart,schemaVersion 升到 12', () => {
    const out = normalizeSettings({
      schemaVersion: 9,
      tts: {
