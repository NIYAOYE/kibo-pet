...
- Produces: `LaunchPlan` 联合类型 + `resolveLaunchPlan(settings: BrowserControlSettings, opts: { cdpPort?: number }): LaunchPlan`,供 Task 3 的 `browserControl.ts`/`playwrightDriver.ts` 使用。
